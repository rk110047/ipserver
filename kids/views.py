...
from .models import KidsCategories,KidsChannels
from .serializers import CategoriesSerializer,ChannelsSerializer
...
class CreateAndListCategoryView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        if user.is_authenticated and user.role == 'admin':
            return KidsCategories.objects.all().order_by("serial_no")

        return KidsCategories.active_objects.all_published().order_by("serial_no")
    ...
    def get(self, request):
        ...
        if user.is_authenticated:
            if chfilter is not None:
                channels = KidsChannels.objects.filter(name__contains=chfilter).order_by("ch_num")
            else:
                channels = KidsChannels.objects.all().order_by("ch_num")
        else:
            return KidsChannels.objects.none()
